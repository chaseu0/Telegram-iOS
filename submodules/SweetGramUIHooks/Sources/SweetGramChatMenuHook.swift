...
public enum SweetGramChatMenuHook {
        ...
        |> mapToSignal { view, _, _ -> Signal<(String, [String]), NoError> in
            ...
            |> map { peer in
                ...
                let lines = view.entries.compactMap { entry -> String? in
                    ...
                    let author = message.author?.compactDisplayTitle ?? "Unknown"
                    return "\(author): \(text)"
                }
    ...
    private static func runSummary(
        ...
        let presentationData = context.sharedContext.currentPresentationData.with { $0 }
        let statusController = OverlayStatusController(theme: presentationData.theme, type: .loading(canceled: nil))
